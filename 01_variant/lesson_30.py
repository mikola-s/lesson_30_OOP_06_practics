    ...
    def buy(self, product: Product, want_to_buy: int = 1):
        ...
        if product.in_stock >= want_to_buy:  # на складе есть
            if self.get_quantity_of_product(product) >= want_to_buy:  # достаточно денег
                self.set_budget(-product.price * want_to_buy * self.get_discount(product))
...
if __name__ == "__main__":
    bread = Product(name='Bread', price=10, in_stock=200, discount=0.8)
    ...
    product = bread
    count = 20
    print(f"у {user.name} {user.budget} денег", end=' | ')
    print(f"на складе {product.in_stock} {product.name}")
    print(f"{user.name} хочет купить {count} {product.name} ")
    print(f"результат {user.buy(product, count)}")
    print(f"у {user.name} {user.budget} денег", end=' | ')
    print(f"на складе {product.in_stock} {product.name}\n")
